...
public class ImageProcessing {
    public static void main(String[] args) {
        ...
        twoDToImage(stretchedH, "./stretchedH_apple.jpg");
        //Shrink Vertically
        int[][] shrinkedV = shrinkVertically(imageData);
        twoDToImage(shrinkedV, "./shrinked_apple.jpg");
        //Invert Image
        int[][] invertedImg = invertImage(imageData);
        twoDToImage(invertedImg, "./inverted_apple.jpg");
        //Colorfilter
        int[][] colorfImg = colorFilter(imageData, -66, 55, -50);
        twoDToImage(colorfImg, "./colorf_apple.jpg");
        ...
        twoDToImage(blankCanvas, "./blankCanvas.jpg");

        int[] rgba = {249, 180, 0, 17};
        int[][] rectangle = paintRectangle(imageData, 400, 500, 14, 18, getColorIntValFromRGBA(rgba));
    ...
    public static int[][] negativeColor(int[][] imageTwoD) {
        ...
        int[][] negCol = new int[imageTwoD.length][imageTwoD[0].length];
        for(int i = 0; i < imageTwoD.length; i++){
            for(int j = 0; j < imageTwoD[i].length; j++){
                ...
                negCol[i][j] = getColorIntValFromRGBA(rgba);
            }
        }
        return negCol;
    }
    ...
    public static int[][] invertImage(int[][] imageTwoD) {
        // TODO: Fill in the code for this method
        int[][] invImg = new int[imageTwoD.length][imageTwoD[0].length];
        for(int i = 0; i < imageTwoD.length; i++){
            for(int j = 0; j < imageTwoD[0].length; j++){
                invImg[i][j] = imageTwoD[(imageTwoD.length-1)-i][(imageTwoD[i].length-1)-j];
            }
        }
        return invImg;
    }
    ...
    public static int[][] colorFilter(int[][] imageTwoD, int redChangeValue, int greenChangeValue, int blueChangeValue) {
        // TODO: Fill in the code for this method
        int[][] colorImg = new int[imageTwoD.length][imageTwoD[0].length];
        for(int i = 0; i < imageTwoD.length; i++){
            for(int j = 0; j < imageTwoD[0].length; j++){
                int[] rgba = getRGBAFromPixel(imageTwoD[i][j]);
                ...
                colorImg[i][j] = getColorIntValFromRGBA(rgba);
            }
        }
        return colorImg;
    }
    ...
    //Paint Random Image-Method
    public static int[][] paintRandomImage(int[][] canvas) {
        // TODO: Fill in the code for this method
        Random rand = new Random();
        for(int i = 0; i < canvas.length; i++){
            for(int j = 0; j < canvas[0].length; j++){
                int[] kannwas = { rand.nextInt(256), rand.nextInt(256), rand.nextInt(256), 255 };
                canvas[i][j] = getColorIntValFromRGBA(kannwas);
            }
        }
        return canvas;
    }

    //Paint Rectangle-Method
    ...
    public static int[][] generateRectangles(int[][] canvas, int numRectangles) {
        ...
        for(int i = 0; i < numRectangles; i++){
            ...
            int randRow = canvas.length - randomHeight;
            int randCol = canvas[0].length - randomWidth;
            int[] rgba = {rand.nextInt(256), rand.nextInt(256), rand.nextInt(256), 255};
            int randomColor = getColorIntValFromRGBA(rgba);
            canvas = paintRectangle(canvas, randomWidth, randomHeight, randRow, randCol, randomColor);
        }
